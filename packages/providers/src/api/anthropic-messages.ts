...
import { parseJsonWithRepair, parseStreamingJson, parseToolCallArguments } from "../utils/json-parse.ts";
...
export const stream: StreamFunction<"anthropic-messages", AnthropicOptions> = (
	...
	(async () => {
		...
		try {
			...
			// `start` marks an empty initial message; deltas build content and the
			// final event is authoritative. `output.content` is mutated in place as
			// chunks arrive and the event stream runs ahead of the consumer, so give
			// `start` its own empty content array rather than aliasing the live one.
				...
				if (event.type === "message_start") {
					...
					if (block) {
						...
						if (block.type === "text") {
							...
							const parsed = parseToolCallArguments(block.partialJson);
							block.arguments = parsed.arguments;
							if (parsed.error) block.argumentsError = parsed.error;
...
function createClient(
	...
	// OAuth bearer auth. Keep the public OAuth transport contract generic; do not
	// impersonate another client or rewrite the caller's tool names.
...
function buildParams(
	...
	if (context.systemPrompt) {
		// Add cache control to the caller's system prompt without injecting a
		// client identity or provider-owned instructions.
		params.system = [
			{
				type: "text",
