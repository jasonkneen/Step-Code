...
import type { AssistantMessageEventStream, ModelsSimpleStreamOptions } from "@step-harness/providers";
import { type Api, clampThinkingLevel, type Message, type Model, streamSimple } from "@step-harness/providers/compat";
...
} from "./model-request-observer.ts";
...
import type { SystemPromptProduct } from "./system-prompt.ts";
import {
	createBashTool,
	createCodingTools,
	createEditTool,
...
export interface CreateAgentSessionOptions {
	...
	model?: Model<any>;
	/** Product fallback provider used only when settings do not select one. */
	defaultProvider?: string;
	/** Product fallback model id used only when settings do not select one. */
 ...
 *
 * // With explicit model
 * import { getModel } from '@step-harness/providers';
 * const { session } = await createAgentSession({
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	const settingsManager =
		options.settingsManager ?? SettingsManager.create(cwd, agentDir, { configDirName: options.configDirName });
	...
	if (!model) {
		const result = await findInitialModel({
			...
			defaultProvider: settingsManager.getDefaultProvider() ?? options.defaultProvider,
			defaultModelId: settingsManager.getDefaultModel() ?? options.defaultModelId,
	...
	const modelRequestObserver = options.modelRequestObserver;
	...
	agent = new Agent({
		...
		streamFn: async (model, context, options) => {
			...
			let statusCode = 0;
			// ModelRuntime can replace the model's base URL after resolving auth
			// (for example an OAuth provider or a configured proxy). Keep the value
			// returned by the provider callback so telemetry classifies the endpoint
			// that actually handled this request.
			let responseModel: Model<Api> = model;
			...
			const requestOptions: ModelsSimpleStreamOptions = {
				...
				...(modelRequestObserver || options?.onResponse
					? {
							onResponse: async (response, resolvedModel: Model<Api>) => {
								statusCode = response.status;
								if (modelRequestObserver) responseModel = resolvedModel;
								await options?.onResponse?.(response, resolvedModel);
							},
						}
					: undefined),
			...
			try {
				stream = modelRuntime.streamSimple(model, context, requestOptions);
			} catch (error) {
				observeModelRequestFailure(model, modelRequestObserver, error, {
					startedAt,
			...
			return observeModelRequestStream(model, stream, modelRequestObserver, {
				startedAt,
				sessionId: options?.sessionId,
				getStatusCode: () => statusCode,
