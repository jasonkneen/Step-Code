...
import type { ModelRequestCompleted, ModelRequestStarted } from "../src/core/model-request-observer.ts";
...
describe("createAgentSession stream options", () => {
	...
	it("passes the Step observer through the native stream function", async () => {
		const started: ModelRequestStarted[] = [];
		const completed: ModelRequestCompleted[] = [];
		const observer = {
			onRequestStarted: (request: ModelRequestStarted) => {
				started.push(request);
			},
			onRequestCompleted: (request: ModelRequestCompleted) => {
